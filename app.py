...
# # ---------------- Home ----------------
# # ---------------- FIXED Home Route with Multi-Semester Support ----------------
...
#         semester_results = {}  # To store results by semester
#         overall_cgpa = None
...
#             # FIXED: Use getlist() and ensure it's a flat list of strings
#             selected_semesters = request.form.getlist('semester')
            
#             # SAFETY: Flatten if needed (in case of nested lists)
#             flat_semesters = []
#             for sem in selected_semesters:
#                 if isinstance(sem, list):
#                     flat_semesters.extend(sem)
#                 else:
#                     flat_semesters.append(str(sem))  # Ensure string
...
#             print(f"DEBUG: Type of selected_semesters: {type(selected_semesters)}")
...
#                 # SAFETY: Ensure semester is string
#                 semester = str(semester).strip()
#                 print(f"DEBUG: Processing semester: '{semester}' (type: {type(semester)})")
...
#                 print(f"DEBUG: Found {len(semester_data)} records for {semester}")
                
#                 if semester_data:
#                     # Calculate SGPA for this semester
#                     sgpa, total_credits = calculate_sgpa(semester_data)
                    
#                     semester_results[semester] = {  # semester is now guaranteed to be string
#                         'data': semester_data,
...
#             print(f"DEBUG: Semester results keys: {list(semester_results.keys())}")
...
#             # FIXED: Route to correct template based on number of selected semesters
#             if len(selected_semesters) == 1:
#                 # Single semester - use existing display.html
#                 semester = str(selected_semesters[0])  # Ensure string
#                 print(f"DEBUG: Single semester check: '{semester}' in {list(semester_results.keys())}")
...
#             else:
#                 # Multiple semesters - use multi-semester template
#                 semesters_display = ", ".join(selected_semesters)
#                 return render_template(
#                     'display_multi.html',
#                     result=results,  # All combined results
#                     semester_results=semester_results,  # Results grouped by semester
...
# # Ensure CBCS indexes for better performance
# def ensure_cbcs_indexes():
#     try:
...
# # ---------------- CBCS Management ----------------
# @app.route('/basket')
# def basket():  # Changed function name from 'cbcs' to 'basket'
...
#         # Build query - Use correct field names from your CSV
#         query = {}
...
#         if search:
#             query['$or'] = [
#                 {'Subject_name': {'$regex': search, '$options': 'i'}},  # From your CSV
#                 {'Subject Code': {'$regex': search, '$options': 'i'}}   # From your CSV (with space)
#             ]
...
#         # Debug: Print to see what fields exist
#         if subjects:
#             print("DEBUG - Sample subject fields:", subjects[0].keys())
        
#         # Get unique branches and baskets for filters
...
#         return render_template('basket.html', error=str(e))  # Fixed template name


# @app.route('/basket/add', methods=['GET', 'POST'])
# def basket_add():  # Changed function name from 'cbcs_add' to 'basket_add'
...
#                 'Subject Code': request.form.get('subject_code', '').strip().upper(),  # With space
#                 'Subject_name': request.form.get('subject_name', '').strip(),          # With underscore
...
# def basket_edit(subject_id):  # Changed function name from 'cbcs_edit' to 'basket_edit'
#     """Edit existing CBCS subject"""
#     try:
#         # Validate ObjectId first
#         from bson import ObjectId
#         if not ObjectId.is_valid(subject_id):  # Added ObjectId validation
...
#                 branches = sorted([b for b in cbcs_collection.distinct('Branch') if b]) or ['All', 'CSE', 'ECE', 'EEE', 'Civil', 'Mechanical']
#                 baskets = sorted([b for b in cbcs_collection.distinct('Basket') if b]) or ['Basket I', 'Basket II', 'Basket III', 'Basket IV', 'Basket V']
#                 return render_template('basket_edit.html', error=error, subject=subject, branches=branches, baskets=baskets)  # Fixed template name
...
#     except Exception as e:
#         print(f"ERROR in basket_edit: {str(e)}")  # Added debug print
#         return redirect(f'/basket?error={str(e)}')


# @app.route('/basket/delete/<subject_id>', methods=['POST'])
# def basket_delete(subject_id):  # Changed function name from 'cbcs_delete' to 'basket_delete'
#     """Delete CBCS subject"""
#     try:
#         from bson import ObjectId
#         if not ObjectId.is_valid(subject_id):  # Added ObjectId validation
...
# # Import CBCS data from your CSV
# @app.route('/basket/import')
# def basket_import():  # Changed function name from 'cbcs_import' to 'basket_import'
#     """Import CBCS data from attached CSV file"""
#     try:
#         import os
#         # Check if CSV file exists
#         csv_path = 'CBCS.csv'  # Changed from 'basket.csv' to 'CBCS.csv' to match your uploaded file
#         if not os.path.exists(csv_path):
#             return f"Error: CSV file '{csv_path}' not found. Please upload your CBCS.csv file to the project directory."
        
#         # Read the CSV data (using your attached file structure)
...
#         records = []
#         for _, row in df.iterrows():
#             # Skip rows with empty subject codes
...
#             # Only add records with valid subject codes
...
# # Debug route to check your data structure
# @app.route('/basket/debug')
# def basket_debug():  # Changed function name from 'cbcs_debug' to 'basket_debug'
...
# # ---------------- Basket Track Route (Fixed) ----------------
# @app.route('/baskettrack', methods=['GET', 'POST'])  # ✅ Added leading slash
# def baskettrack():
#     """Track student progress by connecting CUTM and CBCS clusters"""
#     try:
...
#             if not reg_no:
#                 return render_template('baskettrack.html', error="Please enter a registration number")
            
#             search_performed = True
            
#             # Step 1: Get student's branch for filtering CBCS subjects
#             branch = get_branch_from_reg_no(reg_no)
#             branch_short = branch.split()[0] if branch != 'Unknown Branch' else 'Unknown'
            
#             # Step 2: MongoDB Aggregation Pipeline to join CBCS with CUTM data
#             pipeline = [
#                 # Match CBCS subjects for student's branch
#                 {
#                     "$match": {
#                         "$or": [
#                             {"Branch": "All"},
...
#                     }
#                 },
#                 # Lookup/Join with CUTM collection on Subject Code
#                 {
#                     "$lookup": {
#                         "from": "CUTM1",  # CUTM collection name
#                         "let": {"cbcs_code": "$Subject Code"},  # CBCS Subject Code field
#                         "pipeline": [
#                             {
#                                 "$match": {
#                                     "$expr": {
#                                         "$and": [
#                                             {"$eq": ["$Subject_Code", "$$cbcs_code"]},  # Match subject codes
#                                             {"$eq": ["$Reg_No", reg_no]}  # Match registration number
#                                         ]
#                                     }
#                                 }
#                             },
...
#                     }
#                 },
#                 # Add fields to determine completion status
#                 {
#                     "$addFields": {
#                         "completed": {"$gt": [{"$size": "$student_record"}, 0]},
#                         "grade": {"$arrayElemAt": ["$student_record.Grade", 0]},
#                         "semester": {"$arrayElemAt": ["$student_record.Sem", 0]}
#                     }
#                 },
#                 # Group by Basket for organized display
#                 {
#                     "$group": {
#                         "_id": "$Basket",
#                         "subjects": {
...
#                         "completed_subjects": {"$sum": {"$cond": ["$completed", 1, 0]}}
#                     }
#                 },
#                 # Sort baskets alphabetically
#                 {"$sort": {"_id": 1}}
#             ]
            
#             # Execute aggregation pipeline
#             basket_data = list(cbcs_collection.aggregate(pipeline))
            
#             # Step 3: Calculate overall progress statistics
#             total_subjects = sum(basket['total_subjects'] for basket in basket_data)
#             total_completed = sum(basket['completed_subjects'] for basket in basket_data)
#             overall_percentage = round((total_completed / total_subjects * 100), 1) if total_subjects > 0 else 0
            
#             # Process basket completion data
#             for basket in basket_data:
#                 basket_name = basket['_id']
#                 basket_total = basket['total_subjects']
#                 basket_completed = basket['completed_subjects']
#                 basket_percentage = round((basket_completed / basket_total * 100), 1) if basket_total > 0 else 0
                
#                 basket_completion[basket_name] = {
#                     'subjects': basket['subjects'],
#                     'total': basket_total,
#                     'completed': basket_completed,
#                     'pending': basket_total - basket_completed,
#                     'percentage': basket_percentage
#                 }
            
#             # Step 4: Get additional student info from CUTM cluster
#             student_info = cutm_collection.find_one({"Reg_No": reg_no}, {"Name": 1, "_id": 0})
...
#                 'branch': branch,
#                 'total_subjects': total_subjects,
#                 'completed_subjects': total_completed,
#                 'pending_subjects': total_subjects - total_completed,
#                 'overall_percentage': overall_percentage
#             }
        
#         return render_template('baskettrack.html',
#                              student_progress=student_progress,
#                              basket_completion=basket_completion,
#                              search_performed=search_performed)
                             
#     except Exception as e:
#         print(f"ERROR in baskettrack: {str(e)}")
#         return render_template('baskettrack.html', error=str(e))
